...
function spawnAlien(){
  if(frameCount % 50 === 0 ){
    alien = createSprite(Math.round(random(200,500)),50,20,20);
    alien.addImage("alien",alienImg);
    alien.velocityY=(3 + 10*score/20);
    alien.scale=0.05;
    alien.lifetime=185;
    alienG.add(alien);
  }
...
function spawnMeteor(){
  if(frameCount % 50 === 0 ){
    meteor = createSprite(Math.round(random(100,500)),50,20,20);
    meteor.addImage("meteor",meteorImg);
    meteor.velocityY=(3 + 10*score/50);
    meteor.scale=0.05;
    meteor.lifetime=185;
    meteorG.add(meteor);
  }
